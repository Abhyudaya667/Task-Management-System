...
// // hydrateUsers fetches Assignee + AssignedBy user docs in one DB round-trip.
...
// // canAccess returns true if userID is the assignee or the assigner.
...
// // CreateTask inserts a new task; the caller becomes AssignedBy.
...
// 	assigneeOID, err := primitive.ObjectIDFromHex(req.AssigneeID)
// 	if err != nil {
...
// // GetTaskByID returns full task details; only assigner or assignee may view.
...
// // UpdateTask applies a partial update; only AssignedBy (creator) may edit.
...
// 	// Only the creator (assigned_by) may update
// 	if task.AssignedBy != requesterID {
// 		return nil, ErrUnauthorized
// 	}

// 	// Build $set patch — only include fields that were provided
// 	patch := bson.M{}
...
// 	if len(patch) == 0 {
// 		// Nothing to update — return current detail
...
// 	// Re-fetch to return the up-to-date document
...
// // DeleteTask soft-deletes a task; only AssignedBy (creator) may delete.
...
// // Needed to satisfy interface when time package is imported only for type usage
// var _ = time.Now
package service
